...
exports.fetchProduct = async (req, res) => {
  const { products, total, limit, page } =
    await adminServices.fetchProductService(req, res);
  res.status(200).json({
    success: true,
...
exports.filterProduct = async (req, res) => {
  const { products, total, limit, page } =
    await adminServices.filterProductSevice(req, res);
  res.status(200).json({
    success: true,
    message: "admin fetched filerd products",
    result: {
      data: products,
      pagination: {
        totalItems: total,
...
exports.fetchUser = async (req, res) => {
  const { users, total, limit, page } = await adminServices.fetchUserService(
    req,
    res
  );
  res.status(200).json({
...
exports.fetchOrder = async (req, res) => {
  const { orders, total, limit, page } = await adminServices.fetchOrdersService(
    req,
    res
  );
  res.status(200).json({
